...
def load_players_lc(players_d, day, event_id, file):
    actual_players = event_players(event_id)
    for p in players_d.values():
        exists = p._id in actual_players
        if exists:
            new_line = p._id + ',' + str(day)[:-1] + ',' + str(event_id) + ',' + p.mu + ',' + p.sigma
            ...
            with open('handicap/validation/aga/archivos/learning_curves.csv', 'w') as log_lc:
                ...
                with open(in_filename, 'r') as file:
                    ...
                    for line in file:
                        ...
                        actual_evidence = rango.win_chance_hk(winner_mu, loser_mu, winner_sigma, loser_sigma, float(handicap), float(komi), parameters, black_win)
                        ...
                        if event_id != actual_event: #si cambié de evento
                            ...
                            #cargo las learning_curves
                            load_players_lc(players_dict, prev_end_date, actual_event, log_lc)
                            # reinicio games
                            games = "GAMES\n"
                        ...
                        #acá modifico el days de los jugadores implicados
